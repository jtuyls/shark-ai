...
> **Note:** The `--apply-shuffle` flag is required for the optimized ASM matmul kernel.
...
- **Model Size:** ~40GB for IRPA, ~3MB for compiled VMFB
- **ASM Kernel:** Use `--apply-shuffle` during IRPA creation and `--matmul-kernel="sharktank.asm;*"` during export
